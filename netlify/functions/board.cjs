...
// Defaults match the base Claude set up for you; override via env vars if you
// ever recreate the tables elsewhere.
const TABLES = {
  categories: process.env.AIRTABLE_CATEGORIES_TABLE_ID || "tblJtaeBL6f0HfdYB",
  projects: process.env.AIRTABLE_PROJECTS_TABLE_ID || "tblPIJHE0nvQjyDv4",
  tasks: process.env.AIRTABLE_TASKS_TABLE_ID || "tblJmuviwhjxbfoFO",
};
...
exports.handler = async (event) => {
  ...
  try {
    if (event.httpMethod === "GET") {
      ...
      return { statusCode: 200, body: JSON.stringify({ categories, subcategories, tasks }) };
    }
    ...
    if (event.httpMethod === "POST") {
      ...
      if (!tableId) return { statusCode: 400, body: JSON.stringify({ error: `Unknown table: ${table}` }) };
      ...
      if (op === "create") {
        ...
        return { statusCode: 200, body: JSON.stringify({ id: data.records[0].id }) };
      }
