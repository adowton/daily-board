...
import {
  ...
  Circle, Pencil, RotateCcw, PlayCircle, ArrowUp, ArrowDown, Loader2, Flag, ListChecks, Zap, Flame
} from "lucide-react";
...
export default function App() {
  ...
  async function loadBoard() {
    try {
      ...
      const cleanTasks = (data.tasks || []).map((t) => ({ effort: null, ...t }));
      setCategories(cats);
  ...
  function cycleHealth(subId) {
    ...
    setSubcategories((prev) =>
      prev.map((s) => {
        ...
        const idx = HEALTH_ORDER.indexOf(s.health || "green");
        nextHealth = HEALTH_ORDER[(idx + 1) % HEALTH_ORDER.length];
    ...
    <div className="board-root">
      <style>{`
        ...
        @media (max-width: 720px) {
          ...
          .task-row { flex-wrap: wrap; row-gap: 8px; padding: 10px 8px; }
          .task-text { min-width: 60%; }
